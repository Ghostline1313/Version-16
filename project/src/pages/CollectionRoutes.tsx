...
import { Map, Calendar, Clock, Users, Truck, Plus, MapPin, ArrowRight } from 'lucide-react';
...
const CollectionRoutes = () => {
  ...
  useEffect(() => {
    ...
    fetch('/src/data/Circuit_Collecte_Mermoz_Sacré_Coeur.json')
      ...
      .catch(error => {
        console.error('Error loading collection routes:', error);
        setIsLoading(false);
      });
  }, []);
  ...
  const estimateTime = (length: number) => {
    // Rough estimation: 1km = 12 minutes (slower than sweeping due to stops)
    const minutes = Math.round((length / 1000) * 12);
    if (minutes >= 60) {
  ...
  const handleViewOnMap = (route: CollectionRoute) => {
  ...
  return (
    <div className="h-full flex flex-col">
      <div className="mb-4 flex items-center justify-between">
        <h1 className="text-green-600">Circuits de collecte</h1>
